...
<div class="card card-outline card-primary rounded-0 shadow">
    ...
    <div class="card-footer text-center rounded-0">
            ...
            <?php if($status == 0): ?>
            <a class="btn btn-flat btn-sn btn-primary" href="<?= base_url."?page=manage_registration&id={$id}" ?>" id="print"><i class="fa fa-edit"></i> Edit</a>
		...
		$.ajax({
			...
			success:function(resp){
				if(typeof resp== 'object' && resp.status == 'success'){
					location.replace('./?page=registration_list');
				}else{
					alert_toast("An error occured.",'error');
					end_loader();
				}
